// Text pool (v2) — real-time canvas text with a lingering ripple wake.
//
// Forked from textpool.js: the first `staticWords` words (the founder name)
// are excluded from the cursor mechanic entirely — they are not drawn on the
// canvas and never move. main2.js renders them as real DOM links laid exactly
// over their pool positions, so the name is clickable while the body copy
// around it still stirs.
//
// Layout is done DOM-free with @chenglou/pretext (prepareWithSegments +
// layoutWithLines). Every word is a particle anchored to its laid-out home
// position. The cursor stirs nearby words; a soft spring with low damping
// drags them home slowly, which produces the lingering wave. Disturbance also
// drives per-word "heat": heat makes words MORE TRANSPARENT while the wave
// passes.
...
export class TextPool {
  constructor(canvas, paragraphs, opts = {}) {
    ...
    this.staticWords = opts.staticWords ?? 0            // first N words: DOM links
    this.words = []
    this.revealed = 0
    this.active = false
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const token of lines[i].text.split(' ')) {
          ...
          this.words.push({
            ...
            heat: 0,
            w,
            stat: wordIndex < this.staticWords,
          })
          wordIndex++
          x += w + spaceW
        }
  ...
  draw() {
    ...
    for (let i = 0; i < this.revealed; i++) {
      ...
      if (w.stat) continue // the name renders as a DOM link, not on canvas
